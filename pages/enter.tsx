...
const Enter: NextPage = () => {
	const [enter, { loading, data, error }] = useMuatation<MutationResult>("/api/users/enter");
	const [confirmToken, { loading: tokenLoading, data: tokenData }] = useMuatation<MutationResult>("/api/users/confirm");
				...
				{data?.ok ? (
					...
					<>
						...
						<form onSubmit={handleSubmit(onValid)} className="flex flex-col mt-8 space-y-4">
							...
							{method === "phone" ? (
								<Input register={register("phone", { required: true })} name="phone" label="Phone number" type="number" kind="phone" />
							) : null}
